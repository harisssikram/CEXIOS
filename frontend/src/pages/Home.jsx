...
export default function Home() {
  ...
  return (
    <div className="h-screen bg-[#050b17] relative overflow-y-auto overflow-x-hidden font-sans text-white flex flex-col">
      {/* Ambient glow orbs */}
      ...
      {/* Nav */}
      <header className="relative z-20 shrink-0 px-6 sm:px-10 py-4 flex items-center justify-between">
      ...
      {/* Hero */}
      <main className="relative z-10 flex-1 min-h-0 flex flex-col items-center justify-center px-6 py-2 text-center">
        <motion.p
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
          className="text-accent-400 font-bold text-xs tracking-[0.3em] uppercase mb-3"
        >
        ...
        <motion.h1
          ...
          className="text-4xl sm:text-6xl font-black tracking-tight max-w-4xl leading-[1.05] text-transparent bg-clip-text bg-gradient-to-b from-white via-blue-100 to-accent-400 drop-shadow-[0_0_40px_rgba(30,99,255,0.35)]"
        >
        ...
        <motion.p
          ...
          className="text-white/50 text-base sm:text-lg mt-4 max-w-xl font-light"
        >
        ...
        <motion.div
          ...
          className="grid sm:grid-cols-2 gap-4 mt-8 w-full max-w-2xl"
        >
          {cards.map((card) => (
            <button
              key={card.to}
              onClick={() => navigate(card.to)}
              className="group relative text-left rounded-2xl p-5 border border-white/10 bg-white/[0.03] backdrop-blur-xl hover:border-accent-400/40 hover:-translate-y-1 transition-all duration-300 shadow-[0_8px_32px_rgba(0,0,0,0.35)] hover:shadow-[0_0_40px_-10px_rgba(30,99,255,0.35)] focus-ring overflow-hidden"
            >
              <div
              ...
              />
              <div className="relative w-10 h-10 rounded-xl bg-gradient-to-br from-white/10 to-white/[0.02] border border-white/10 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300">
                <card.icon className="w-5 h-5 text-accent-400 drop-shadow-[0_0_10px_rgba(62,123,255,0.7)]" />
              </div>
              <h3 className="relative font-semibold text-base mb-1 flex items-center gap-1.5 tracking-wide">
                {card.title}
                <ArrowRightIcon className="w-4 h-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all" />
              </h3>
              <p className="relative text-xs text-white/45 leading-relaxed">{card.description}</p>
            </button>
          ))}
        </motion.div>
      ...
      <footer className="relative z-10 shrink-0 px-6 py-3 text-center text-white/30 text-xs tracking-wide">
